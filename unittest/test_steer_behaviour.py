...
from steer.steer_behaviour import flee
from steer.steer_behaviour import pursuit
...
class TestSteerBehaviour(unittest.TestCase):
    def test_seek(self):
        ...
        w.pos = Vector(
            30, 9
        )  # change only the waypoint, expecting to get the full max speed
        self.assertAlmostEqual(f(e, w).length(), e.max_force)
        f = seek(
            (w.pos - e.pos).length() * slow_radius_factor
        )  # getting the force function again with new slow radius
        self.assertAlmostEqual(f(e, w).length(), e.max_force)  # / slow_radius_factor)
    ...
    def test_update_steer_behaviour(self):
        ...
        self.assertAlmostEqual(
            e.velocity.length(),
            ((w.pos - e.pos).normalize() * e.max_force * 0.1).length(),
        )
